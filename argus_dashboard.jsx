...
const MOCK_RULES = [
  { id: 'r1', name: 'Block Tor Exit Nodes', action: 'deny', direction: 'inbound', srcCidr: '', dstCidr: '', protocol: 'any', port: '', priority: 10, enabled: true },
  { id: 'r2', name: 'Allow Internal DNS', action: 'allow', direction: 'outbound', srcCidr: '10.0.0.0/8', dstCidr: '', protocol: 'udp', port: '53', priority: 20, enabled: true },
  { id: 'r3', name: 'Rate Limit SSH Attempts', action: 'rate-limit', direction: 'inbound', srcCidr: '0.0.0.0/0', dstCidr: '', protocol: 'tcp', port: '22', priority: 30, enabled: true },
  { id: 'r4', name: 'Deny Bogon Ranges', action: 'deny', direction: 'inbound', srcCidr: '', dstCidr: '', protocol: 'any', port: '', priority: 5, enabled: true },
  { id: 'r5', name: 'Allow HTTPS Outbound', action: 'allow', direction: 'outbound', srcCidr: '', dstCidr: '0.0.0.0/0', protocol: 'tcp', port: '443', priority: 40, enabled: true },
  { id: 'r6', name: 'Block AbuseIPDB Feeds', action: 'deny', direction: 'inbound', srcCidr: '', dstCidr: '', protocol: 'any', port: '', priority: 1, enabled: false },
  { id: 'r7', name: 'Allow ICMP Monitoring', action: 'allow', direction: 'inbound', srcCidr: '10.0.0.0/8', dstCidr: '', protocol: 'icmp', port: '', priority: 50, enabled: true },
];

const MOCK_EVENTS = [
  { id: 'e1', type: 'block', msg: 'Blocked 185.220.101.42 → 192.168.1.1:22 (Tor exit node)', time: '14:23:04', sev: 'critical' },
  { id: 'e2', type: 'alert', msg: 'Rate limit exceeded: 203.0.113.47 → 192.168.1.1:22 (850 req/min)', time: '14:22:58', sev: 'warning' },
  { id: 'e3', type: 'info', msg: 'Connection ESTABLISHED: 10.0.0.23:45123 → 8.8.8.8:53', time: '14:22:55', sev: 'info' },
  { id: 'e4', type: 'block', msg: 'Threat intel hit: 45.33.32.156 on Spamhaus DROP list', time: '14:22:50', sev: 'critical' },
  { id: 'e5', type: 'info', msg: 'Rule applied: "Allow Internal DNS" matched 10.0.0.23→8.8.8.8:53', time: '14:22:45', sev: 'info' },
];

const MOCK_CONNS = [
  { id: 'c1', src: '10.0.0.23', sp: 45123, dst: '8.8.8.8', dp: 53, proto: 'UDP', state: 'established', dur: 245, pkts: 127 },
  { id: 'c2', src: '192.168.1.105', sp: 54321, dst: '142.250.80.46', dp: 443, proto: 'TCP', state: 'established', dur: 1890, pkts: 3421 },
  { id: 'c3', src: '10.0.0.45', sp: 22, dst: '203.0.113.47', dp: 51023, proto: 'TCP', state: 'new', dur: 3, pkts: 7 },
  { id: 'c4', src: '192.168.1.1', sp: 443, dst: '10.0.0.23', dp: 45124, proto: 'TCP', state: 'established', dur: 567, pkts: 890 },
];

const MOCK_AUDIT = [
  { id: 'a1', ts: '14:22:30', actor: 'admin', action: 'rule.create', resource: 'firewall', ip: '192.168.1.100', success: true, hash: 'a7f3c2e1d4b5...' },
  { id: 'a2', ts: '14:20:15', actor: 'admin', action: 'login.success', resource: 'auth', ip: '192.168.1.100', success: true, hash: 'b8e4d3f2a5c6...' },
  { id: 'a3', ts: '14:18:00', actor: 'operator', action: 'block.add', resource: 'threat', ip: '10.0.0.1', success: true, hash: 'c9f5e4a3b6d7...' },
  { id: 'a4', ts: '14:15:45', actor: 'unknown', action: 'login.failed', resource: 'auth', ip: '185.220.101.42', success: false, hash: 'd0a6f5b4c7e8...' },
];

const MOCK_REPUTATION = [
  { ip: '185.220.101.42', score: -85, source: 'Spamhaus DROP', lastSeen: '2m ago' },
  { ip: '45.33.32.156', score: -72, source: 'AbuseIPDB', lastSeen: '5m ago' },
  { ip: '203.0.113.47', score: -45, source: 'Scan Detected', lastSeen: '30s ago' },
  { ip: '91.240.118.20', score: -30, source: 'Spamhaus EDROP', lastSeen: '1h ago' },
];

const IPs = ['192.168.1.105', '10.0.0.23', '203.0.113.47', '45.33.32.156', '185.220.101.42', '8.8.8.8', '142.250.80.46'];
const PROTO = ['TCP', 'UDP', 'ICMP'];
const STATES = ['new', 'established', 'closing', 'closed'];
const ACTIONS = ['rule.create', 'rule.delete', 'login.success', 'login.failed', 'block.add', 'block.remove', 'backup.create', 'user.create'];
const EVT_TYPES = ['block', 'alert', 'info'];
...
function useLiveSimulation() {
  const [packetHistory, setPacketHistory] = useState(() => {
    const arr = [];
    for (let i = 0; i < 30; i++) arr.push({ time: `${i}s`, allowed: rnd(500, 3000), dropped: rnd(10, 200) });
    return arr;
  });
  const [stats, setStats] = useState({ allowed: 2847123, dropped: 89234, connections: 47, blocked: 183 });
  const [events, setEvents] = useState(MOCK_EVENTS);
  const [conns, setConns] = useState(MOCK_CONNS);
  ...
  useEffect(() => {
    const intervals = [
      setInterval(() => {
        const da = rnd(100, 500);
        const dd = rnd(0, 50);
        setPacketHistory(prev => [...prev.slice(1), { time: `${rnd(1, 60)}s`, allowed: rnd(500, 3000), dropped: rnd(10, 200) }]);
        setStats(s => ({ ...s, allowed: s.allowed + da, dropped: s.dropped + dd }));
      }, 2000),
      setInterval(() => {
        const newConn = { id: `c${Date.now()}`, src: pick(IPs), sp: rnd(1024, 65535), dst: pick(IPs), dp: rnd(1, 65535), proto: pick(PROTO), state: 'new', dur: 0, pkts: 1 };
        setConns(prev => [newConn, ...prev].slice(0, 20));
      }, 5000),
      setInterval(() => {
        const ev = { id: `e${Date.now()}`, type: pick(EVT_TYPES), msg: `Event ${Date.now()} — ${pick(['Packet blocked','Rule matched','Connection opened','Scan detected'])}`, time: new Date().toLocaleTimeString(), sev: pick(SEVERITIES) };
        setEvents(prev => [ev, ...prev].slice(0, 20));
      }, 4000),
      setInterval(() => {
        setConns(prev => prev.map(c => c.state === 'new' && Math.random() > 0.7 ? { ...c, state: 'established', dur: rnd(10, 60) } : c));
      }, 8000),
      setInterval(() => {
        if (Math.random() > 0.7) {
          const alertMsg = pick(['Port scan detected — 47 ports in 8s', 'Rate limit exceeded - 850 req/min', 'Threat intel hit on Spamhaus DROP', 'Anomaly spike: 8.5x stddev', 'WAN failover activated']);
          setAlerts(prev => [{ id: `al${Date.now()}`, msg: alertMsg, sev: 'critical', time: Date.now() }, ...prev].slice(0, 5));
        }
      }, 15000),
      setInterval(() => {
        const entry = { id: `a${Date.now()}`, ts: new Date().toLocaleTimeString(), actor: pick(['admin', 'operator', 'system']), action: pick(ACTIONS), resource: pick(['firewall', 'auth', 'threat', 'backup']), ip: pick(IPs), success: Math.random() > 0.3, hash: `${Math.random().toString(16).slice(2, 14)}...` };
  ...
  return { packetHistory, stats, events, conns, alerts, auditLog };
}
...
function Dashboard({ data }) {
  ...
  const trendAllowed = ((packetHistory[packetHistory.length - 1]?.allowed || 0) - (packetHistory[0]?.allowed || 0)) / (packetHistory[0]?.allowed || 1) * 100;
  const trendDropped = ((packetHistory[packetHistory.length - 1]?.dropped || 0) - (packetHistory[0]?.dropped || 0)) / (packetHistory[0]?.dropped || 1) * 100;
  ...
  return (
    <div style={{ animation: 'fadeSlideIn 0.3s ease-out' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 20 }}>
        <StatCard label="Packets Allowed" value={stats.allowed} color={COLORS.green} trend={trendAllowed} />
        <StatCard label="Packets Dropped" value={stats.dropped} color={COLORS.red} trend={trendDropped} />
        <StatCard label="Active Connections" value={stats.connections} color={COLORS.cyan} />
        <StatCard label="Blocked IPs" value={stats.blocked} color={COLORS.yellow} />
      </div>
        ...
        <div style={S.card}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              ...
              ['DB', stats.connections > 0 ? COLORS.green : COLORS.yellow],
            ].map(([label, color]) => (
...
function Rules({ rules, onToggle, onDelete, onDuplicate }) {
  ...
  const handleSimulate = () => {
    const matched = rules.find(r => r.enabled && (r.protocol === 'any' || r.protocol === simForm.protocol) && (r.dstCidr === '0.0.0.0/0' || !r.dstCidr));
    setSimResult({ matched: !!matched, rule: matched || null, action: matched ? matched.action : 'allow (default)' });
  };
    ...
    <div style={{ animation: 'fadeSlideIn 0.3s ease-out' }}>
      ...
      {showSimulator && (
        <div style={{ ...S.card, marginTop: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={handleSimulate} style={S.btn('#00d4ff','#080b10')}>▶ SIMULATE</button>
            {simResult && (
              <div style={{ padding: '8px 16px', borderRadius: 6, background: simResult.matched ? 'rgba(0,212,255,0.08)' : 'rgba(255,184,0,0.08)', border: `1px solid ${simResult.matched ? COLORS.cyan : COLORS.yellow}`, fontSize: 12 }}>
                {simResult.matched
                  ? <>Matched: <strong>{simResult.rule.name}</strong> → Action: <Badge type={simResult.action === 'deny' ? 'deny' : 'allow'} label={simResult.action.toUpperCase()} /></>
      ...
      {showNewModal && (
        <Modal onClose={() => setShowNewModal(false)}>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 24 }}>
            ...
            <button onClick={() => { if (validateForm()) { setShowNewModal(false); }}}
              style={S.btn('#00d4ff','#080b10')}>▶ DEPLOY RULE</button>
          </div>
        </Modal>
      )}
    </div>
...
function Alerts({ alerts, onAck }) {
  const [alertRules] = useState([
    { id: 'ar1', name: 'Blocked IP Threshold', condition: '>50 blocked IPs', channels: ['Slack', 'Discord'], enabled: true, cooldown: 300 },
    { id: 'ar2', name: 'Anomaly Spike', condition: 'z-score > 3.0', channels: ['Email'], enabled: true, cooldown: 600 },
  ]);
...
function Audit({ auditLog }) {
  ...
  const filtered = useMemo(() => {
    return auditLog.filter(a => {
      if (filterActor && !a.actor.includes(filterActor)) return false;
      if (filterAction && a.action !== filterAction) return false;
      return true;
    });
  }, [auditLog, filterActor, filterAction]);
  ...
  return (
    <div style={{ animation: 'fadeSlideIn 0.3s ease-out' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => setVerified(null)} style={S.btn('#141923','#7a8fa8')}>📥 Export JSON</button>
          <button onClick={() => { setVerified('loading'); setTimeout(() => setVerified(true), 1500); }} style={S.btn('#b06bff','#fff')}>✓ Verify Integrity</button>
        </div>
      </div>
...
function Reputation() {
  ...
  const handleLookup = () => {
    const found = MOCK_REPUTATION.find(r => r.ip === searchIp);
    if (found) setLookupResult(found);
    else setLookupResult({ ip: searchIp, score: rnd(-30, 30), source: 'New IP', lastSeen: 'Just now' });
  };
        ...
        <div style={S.card}>
          ...
          {lookupResult && (
            <div style={{ marginTop: 16, padding: 16, background: '#141923', borderRadius: 8, border: '1px solid #1e2a38', animation: 'fadeSlideIn 0.3s ease-out' }}>
              ...
              {lookupResult.score < 0 && <button style={{ ...S.btn('rgba(255,59,92,0.12)','#ff3b5c'), marginTop: 12 }}>🚫 Block This IP</button>}
            </div>
          )}
        </div>
            ...
            <tbody>
              {MOCK_REPUTATION.map(r => (
                <tr key={r.ip} onMouseEnter={e => e.currentTarget.style.background='#141923'} onMouseLeave={e => e.currentTarget.style.background='transparent'}>
                  ...
                  <td style={S.td}><button style={S.btn('rgba(255,59,92,0.12)','#ff3b5c')}>🚫 Block</button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function Settings() {
  ...
  const [users, setUsers] = useState([
    ...
    { username: 'operator1', role: 'Operator', enabled: true },
    { username: 'viewer1', role: 'Viewer', enabled: false },
  ]);
  const [backups, setBackups] = useState([]);
  const [vpnRequests, setVpnRequests] = useState([
    { requester: 'user@example.com', submitted: '2024-01-15 14:23', status: 'pending', key: 'MIIBpA...' },
  ]);
  const [clusterNodes, setClusterNodes] = useState([
    { id: 'node-1', ip: '192.168.1.10', role: 'Leader', healthy: true, lastHearbeat: '2s ago' },
    { id: 'node-2', ip: '192.168.1.11', role: 'Follower', healthy: true, lastHearbeat: '1s ago' },
  ]);
  const [syslogConfigs, setSyslogConfigs] = useState([]);
  const [qosPolicies, setQosPolicies] = useState([
    { id: 'q1', name: 'Limit Guest VLAN', target: 'VLAN 100', bandwidth: '50 Mbps', priority: 3, enabled: true },
  ]);
  ...
  const handleCreateBackup = () => {
    ...
    setTimeout(() => {
      const now = new Date();
      const name = `argus-backup-${now.toISOString().slice(0, 19).replace(/[T:]/g, '-')}.json`;
      setBackups(prev => [{ id: `b${Date.now()}`, name, timestamp: now.toLocaleString(), size: '247 KB', checksum: 'a7f3c2e1d4b5...' }, ...prev]);
  ...
  const handleAddUser = () => {
    ...
    setUsers(prev => [...prev, { username: newUser.username, role: newUser.role, enabled: true }]);
  ...
  const handleApproveVpn = (idx) => {
    setVpnRequests(prev => prev.map((r, i) => i === idx ? { ...r, status: 'approved' } : r));
  };

  const handleDenyVpn = (idx) => {
    setVpnRequests(prev => prev.map((r, i) => i === idx ? { ...r, status: 'denied' } : r));
  };
  ...
  return (
    <div style={{ animation: 'fadeSlideIn 0.3s ease-out' }}>
            ...
            <tbody>
              {users.map(u => (
                <tr key={u.username} onMouseEnter={e => e.currentTarget.style.background='#141923'} onMouseLeave={e => e.currentTarget.style.background='transparent'}>
                  ...
                  <td style={S.td}><Toggle on={u.enabled} onChange={() => setUsers(prev => prev.map(x => x.username === u.username ? {...x,enabled:!x.enabled} : x))} /></td>
                  <td style={{ ...S.td, color: '#7a8fa8' }}>{u.username === 'admin' ? '2m ago' : 'Never'}</td>
      ...
      {tab === 'VPN Portal' && (
        <div style={S.card}>
          <h3 style={{ fontSize: 12, color: '#7a8fa8', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 16 }}>WireGuard Peer Requests — {vpnRequests.length} pending</h3>
          <table style={S.table}>
            ...
            <tbody>
              {vpnRequests.map((r, i) => (
                <tr key={r.requester + i}>
                  ...
                  <td style={{ ...S.td, color: '#3d5068' }}>{r.key.slice(0, 16)}...</td>
                  ...
                  <td style={S.td}>
                    {r.status === 'pending' && <div style={{ display: 'flex', gap: 4 }}>
                      <button onClick={() => handleApproveVpn(i)} style={S.btn('rgba(0,255,136,0.12)','#00ff88')}>✓ Approve</button>
                      <button onClick={() => handleDenyVpn(i)} style={S.btn('rgba(255,59,92,0.12)','#ff3b5c')}>✕ Deny</button>
                    </div>}
                    {r.status === 'approved' && <button style={S.btn('#00d4ff','#080b10')}>📥 Download Config</button>}
                  </td>
                </tr>
              ))}
                ...
                {backups.map(b => (
                  <tr key={b.id}>
                    ...
                    <td style={S.td}><div style={{ display: 'flex', gap: 4 }}>
                      <button style={S.btn('#141923','#00d4ff')}>📥 Download</button>
                      <button style={S.btn('rgba(255,184,0,0.12)','#ffb800')}>↺ Restore</button>
                    </div></td>
                  </tr>
                ))}
              </tbody>
            ...
            <tbody>
              {qosPolicies.map(q => (
                <tr key={q.id}>
                  ...
                  <td style={S.td}><Toggle on={q.enabled} onChange={() => setQosPolicies(prev => prev.map(x => x.id === q.id ? {...x,enabled:!x.enabled} : x))} /></td>
...
function Sidebar({ page, onNavigate, unreadAlerts }) {
    ...
    <div style={S.sidebar}>
      ...
      <div style={S.userSection}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          ...
          <span style={{ color: '#e8edf3', fontFamily: '"Inter",system-ui,sans-serif' }}>admin</span>
          <Badge type="purple" label="ADMIN" />
        </div>
        <button style={{ ...S.btn('transparent','#7a8fa8'), fontSize: 11, padding: 0, width: '100%', justifyContent: 'flex-start' }}>⏻ Logout</button>
      </div>
    </div>
  );
}
...
export default function ArgusDashboard() {
  ...
  const renderPage = () => {
    try {
      switch (page) {
        ...
        case 'rules': return <Rules rules={MOCK_RULES} onToggle={handleToggle} onDelete={handleDelete} onDuplicate={handleDuplicate} />;
        case 'connections': return <Connections conns={data.conns} />;
        case 'alerts': return <Alerts alerts={localAlerts} onAck={handleAck} />;
        case 'audit': return <Audit auditLog={data.auditLog} />;
  ...
  return (
    <div style={S.root}>
      <Sidebar page={page} onNavigate={setPage} unreadAlerts={localAlerts.length} />
      <main style={S.main}>
        {renderPage()}
      </main>
